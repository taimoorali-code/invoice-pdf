<!doctype html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Invoice</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <style>
        body {
            font-size: 13px;
        }
    </style>
</head>

<body>
    
    <div class="container">
        <div class="row mt-4">
            <div class="col-4">
                <h1 style="color: #d18a7d" class="mb-4 text-custom">{{ __('invoice.invoice_title') }}</h1>
                <div class="mb-3">
                    <div class="col">
                        <span style="color: #d18a7d" class="text-custom">{{ __('invoice.invoice') }}</span>
                    </div>
                    <div class="col text-right">
                        <span class="bold">{{ $order->id }}</span>
                    </div>
                </div>
                <div class="">
                    <div class="col">
                        <span style="color: #d18a7d" class="text-custom">{{ __('invoice.issue_date') }}</span>
                    </div>
                    <div class="col text-right">
                        <span class="bold">{{ $order->created_at->format('M. d, Y') }}</span>
                    </div>
                </div>
            </div>
            <div class="col-4  p-3" style="border-left: 2px solid #d18a7d;">
                <span class="bold" style="color: #d18a7d">{{ __('invoice.sold_by') }}</span>
                <div style="display: flex; flex-flow: column;" class="my-2">
                    <span class="bold">Noha Nabil Beauty FZCo</span>
                    <span>Office No. B34BS33O002</span>
                    <span>Jafza 22 Building</span>
                    <span>Jebel Ali Free Zone</span>
                    <span>Dubai</span>
                    <span>DU</span>
                    <span>United Arab Emirates</span>
                    <span>TRN: 100454885300003</span>
                </div>
                <div style="display: flex; flex-flow: column; margin-top: 40px">
                    <span class="bold" style="color: #d18a7d">CLIENT</span>
                    <span class="bold">{{ $order->shippingAddress->first_name }} {{ $order->shippingAddress->last_name }}</span>
                    <span>{{ $order->shippingAddress->country }}</span>
                    <span>{{ $order->shippingAddress->city }}</span>
                    <span>{{ $order->shippingAddress->address1 }}</span>
                    <span>{{ $order->shippingAddress->zip }}</span>
                </div>
            </div>
            <div class="col-3  p-3">
                <img width="122" height="55" src="data:image/png;base64," alt="Company Logo" />
            </div>
        </div>
    </div>

    <div class="container mt-4">
        <div class="table-responsive">
            <table class="table">
                <thead style="border-bottom: 3px solid #d18a7d;">
                    <tr>
                        <th style="color: #d18a7d; width: 30%;">{{ __('invoice.item') }}</th>
                        <th style="color: #d18a7d; width: 20%;">{{ __('invoice.description') }}</th>
                        <th style="color: #d18a7d; width: 10%;">{{ __('invoice.quantity') }}</th>
                        <th style="color: #d18a7d; width: 10%;">{{ __('invoice.unit_price') }}</th>
                        <th style="color: #d18a7d; width: 10%;">{{ __('invoice.discount') }}</th>
                        <th style="color: #d18a7d; width: 10%;">{{ __('invoice.tax_amount') }}</th>
                        <th style="color: #d18a7d; width: 10%;">{{ __('invoice.total') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($order->lineItems as $item)
                    <tr>
                        <td>{{ $item->title }}</td>
                        <td>{{ $item->description }}</td>
                        <td>{{ $item->quantity }}</td>
                        <td>{{ $item->price }}</td>
                        <td>{{ $order->current_total_discounts }}%</td>
                        <td>{{ $order->current_total_tax }}د.إ</td>
                        <td>{{ $item->quantity * $item->price }}</td>
                    </tr>
                    @endforeach
                   
                    <tr>
                        <td colspan="2"></td> 
                        <td colspan="3" style="vertical-align: middle;">{{ __('invoice.total') }}</td>
                        <td colspan="2" style="text-align: right; vertical-align: middle;">{{ $order->current_total_price }}</td>
                    </tr>
                    <tr>
                        <td colspan="2"></td> 
                        {{-- total amount paid is not given in the order json file thats why i not mention it --}}
                        <td colspan="3" style="vertical-align: middle;">{{ __('invoice.amount_paid') }}</td>
                        <td colspan="2" style="text-align: right; vertical-align: middle;">0.00 د.إ</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

    <div class="container mt-4">
        <div class="row" style="border: 1px dotted #d18a7d;">
            <div class="col-md-3" style="width: 25%; padding: 20px 4px; color: #d18a7d">{{ __('invoice.issue_date') }}:</div>
            <div class="col-md-3 fs-4" style="width: 25%; padding: 20px 4px; color: #d18a7d">{{ $order->created_at->format('M. d, Y') }}</div>
            <div class="col-md-3" style="width: 25%; padding: 20px 4px; color: #d18a7d">{{ __('invoice.amount_due') }}:</div>
            <div class="col-md-3 fs-4" style="width: 25%; padding: 20px 20px; text-align: right; color: #d18a7d">{{ $order->current_total_price }} إ.د</div>
        </div>
    </div>

    <div style="background: #e7e6e6">
        <div class="container mt-5">
            <div class="p-4">
                <div class="row">
                    <div class="col-8">
                        <span>{{ __('invoice.thank_you_for_your_purchase') }}</span>
                    </div>
                    <div class="col-4" style="border-left: 2px solid #cecdcdb0;">
                        <div style="display: flex; flex-flow: column;">
                            <span class="bold" style="color: #d18a7d">{{ __('invoice.payment_method') }}</span>
                            <span class="bold">Credit Card</span>
                        </div>
                        <div style="display: flex; flex-flow: column; margin-top: 10px">
                            <span class="bold" style="color: #d18a7d">{{ __('invoice.order_number') }} </span>
                            <span>#{{ $order->confirmation_number }}</span>
                        </div>
                    </div>
                </div>
                <div class="container mt-5" style="border-top: 2px solid #cecdcdb0;">
                    <div>
                        <div class="row mt-4">
                            <div class="col" style="border-right: 2px solid #cecdcdb0;">
                                <span class="font-weight-bold" style="color: #d18a7d">Noha Nabil Beauty FZCO</span>
                            </div>
                            <div class="col" style="border-right: 2px solid #cecdcdb0;">
                                <span class="font-weight-bold">Phone: +9718006642</span>
                            </div>
                            <div class="col" style="border-right: 2px solid #cecdcdb0;">
                                <span>Email: customercare@nnbeauty.com</span>
                            </div>
                            <div class="col" style="border-right: 2px solid #cecdcdb0;">
                                <span>Website: nohanabil.com</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mt-2" style="display: flex; justify-content: center; align-items: center">
                    <span>Facebook: facebook.com/NohaNabilBeauty</span>
                </div>
                <div class="mt-4" style="display: flex; justify-content: center; align-items: center;">
                    <span>View this document online at https://noha-nabil.sufio.com/cxqy9zseba4q?fekfcevfcg.</span>
                </div>
            </div>
        </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzJjWckZ+1gLRDkzX+nJ6be6n2"
        crossorigin="anonymous"></script>
</body>

</html>
